...
printf "Masukkan port Nuxt [3000]: "
read -r APP_PORT
...
node -e "const fs=require('fs'); const p='www/$APP_NAME/package.json'; const pkg=JSON.parse(fs.readFileSync(p,'utf8')); pkg.scripts=pkg.scripts||{}; pkg.scripts.dev='nuxt dev --port $APP_PORT'; pkg.scripts.preview='nuxt preview --port $APP_PORT'; fs.writeFileSync(p, JSON.stringify(pkg,null,2)+'\n');"
